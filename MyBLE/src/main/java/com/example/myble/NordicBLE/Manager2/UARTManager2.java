...
public class UARTManager2 extends LoggableBleManager2<UARTManagerCallbacks2> {
    ...
    private static UARTManager2 managerInstance = null;

    public static synchronized UARTManager2 getInstance(final Context context) {
        if (managerInstance == null) {
            managerInstance = new UARTManager2(context);
        }
...
//            BluetoothLeGattServer.getInstance(MainActivity.shared().getApplicationContext()).start();
            Common.wait(100);
            Log.e("TAG", "broadcastUpdate: Time added 2" );
...
//                .enqueue();
        ...
        Log.e("Connection_Manager2", "broadcastUpdate: PktTime Ack sent" );


        writeCharacteristic(rxCharacteristic,bytes,BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE).done(device ->
                Log.e("Connection_Manager2", "broadcastUpdate: PktTime Ack sent Sucessfully" )
        ).fail((device, status) ->
                        Log.d("UARTmanager2", "send: UART Failedto send")
               ).enqueue();
